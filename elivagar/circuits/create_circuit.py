...
from torchquantum.operator import Operation
...
class TQCirc(tq.QuantumModule):
    def __init__(self, gates, gate_params, inputs_bounds,
        ...
        self.embed_gates = []
        self.var_gates = []
        self.ent_gates = []#entanglement gates
        self.gate_params = []
        self.embed_flags = []
        self.param_flags = []
        self.gate_wires = []
        ...
        self.init_params = np.random.normal(loc=0.0, scale=np.pi/2, size=len(gates))
        ...
        for i in range(len(gates)):
            ...
            if weights_bounds[i] != weights_bounds[i + 1]:                
                self.var_gates.append(trainable_mapping[gates[i]](has_params=True, trainable=True,init_params=self.init_params[i]))
    ...
    def forward(self, x):
        ...
        device= tq.QuantumDevice(n_wires=self.n_wires,bsz=x.shape[0])
        ...
        for i in range(self.num_gates):
            if self.embed_flags[i]:
                if self.gate_list[i] == 'amp_enc':
                    self.embed_gates[emb_ind](device, x)
                    continue 
                
                self.embed_gates[emb_ind](device, wires=self.gate_wires[i],
                    params=x[:, self.gate_params[emb_ind]])
                emb_ind += 1
            elif self.param_flags[i]:
                self.var_gates[var_ind](device, wires=self.gate_wires[i])
                var_ind += 1
            else:
                self.ent_gates[ent_ind](device, wires=self.gate_wires[i])
                ent_ind += 1 

        assert emb_ind == x.shape[1]
        ...
        #meas = tq.measurement.expval_joint_analytical(device, observable="IZII")
        #meas=meas.mean(1)
