...
def get_circ_params(dir_path):
    with open(f"{dir_path}/result.pkl", "rb") as f:
        data = pickle.load(f)
    gates=data['gates']
    gate_params=data['gate_params']
    inputs_bounds=data['inputs_bounds']
    weights_bounds=data['weights_bounds']
    quit_mapping=data['qubit_mapping']
    gates = list(filter(lambda x: True if x != '' else False, gates))
    
    return gates, gate_params, inputs_bounds, weights_bounds, quit_mapping