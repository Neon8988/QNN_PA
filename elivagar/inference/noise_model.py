...
def get_params_from_tq_model(model_path, num_params,num_module):
    """
    Get the parameters stoerd in a torch quantum model.
    """
    model_params=torch.load(model_path)
    circuit_modules=[]
    for item in range(num_module):
        circuit_params = np.zeros(num_params)
        for i in range(num_params):
            circuit_params[i] = model_params[f'model{item+1}.var_gates.{i}.params'].numpy().item()
        circuit_modules.append(circuit_params)
    return circuit_modules
...
def run_qiskit_circ(circuit, dev, num_meas_qubits, num_shots=1024, transpile_circ=False, basis_gates=None,
    ...
    if transpile_circ:
        circuit = transpile(circuit, basis_gates=basis_gates, coupling_map=coupling_map,
                           initial_layout=list(qubit_mapping), optimization_level=opt_level,seed_transpiler=42)
