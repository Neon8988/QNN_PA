...
from elivagar.utils.create_noise_models import get_noise_model # get_real_backend_dev, noisy_dev_from_backend, 
...
def compute_clifford_nr_for_circuits(circ_dir, num_circs, device_name,
    ...
    circ_index_offset=0,size=16,use_real_backend=False):
    """
    Compute the Clifford Noise Robustness for a group of circuits stored in the same directory.
    """
    x_train, _ ,_,_,_= load_dataset(dataset, embed_type, data_reps,size)
    ...
    for i in range(circ_index_offset, circ_index_offset + num_circs):
        ...
        print(f'circ_{i+1}', noise_metric_scores[-1])
    ...
    with open(f"{circ_dir}/noise.pkl", "wb") as f:
        pkl.dump(noise_metric_scores, f)
